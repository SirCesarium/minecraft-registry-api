use crate::{error::ApiError, forge::ForgeClient};
...
impl ForgeClient {
    ...
    pub async fn download_installer(&self, mc_version: &str, forge_version: &str) -> Result<Vec<u8>, ApiError> {
        let url = format!(
...
mod tests {
    ...
    async fn test_download_installer() {
        ...
        let jar = client.download_installer("1.21.4", &latest).await.unwrap();
