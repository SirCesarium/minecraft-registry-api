use crate::{
    error::ApiError,
    fabric::{FabricClient, models::InstallerQuery},
};
...
impl FabricClient {
    ...
    /// Downloads a `Fabric` installer JAR, calling `on_chunk` for each received chunk.
    ///
...
mod tests {
    ...
    async fn test_download_installer() {
        ...
        let jar = client
            .download_installer(InstallerQuery { version: "1.1.1" })
            .await
            .unwrap();
